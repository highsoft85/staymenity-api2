...
class ImageUploadIdentityBackService extends ImageUploadService implements ImageUploadInterface
{
    /**
    ...
    {
        $oMainImages = $oItem->imagesIdentityBack()->where('is_main', 1)->get();
    ...
    public function delete(object $oItem, Image $oImage, array $options): bool
    {
        ...
        $this->deleteImages($oImage->filename, $key, $oItem->id, $filters, ImageType::IDENTITY_TYPE_BACK);
        $oImage->delete();
        return true;
    }
    ...
    public function upload(object $oFile, object $oItem, array $options): string
    {
        $key = $oItem->getImageTypeModel();
        $filters = isset($options['filters']) ? $options['filters'] : [];
        $filename = $this->uploadFile($oFile, $key, $oItem->id, $filters, ImageType::IDENTITY_TYPE_BACK);
        $oItem->imagesIdentityBack()->create([
