...
class CasesBarbequeNewYorkCommand extends Command
{
    use CommandTrait;
     ...
     *
     */
    const SIGNATURE = 'cases:barbeque-new-york';
    ...
    {
        $hostEmail = 'host-3@admin.com';

        $oUser = User::where('email', $hostEmail)->first();
        if (!is_null($oUser)) {
            (new UserServiceModel($oUser))->delete();
        }
        ...
        imageUploadUser(storage_path('tests/user/barbeque-man.jpg'), $oHost, ImageType::MODEL, true);
        $oType = Type::where('name', 'barbecue')->first();
        ...
        imageUpload(storage_path('tests/listings/barbeque-11.jpg'), $oListing, ImageType::MODEL, [], [], true);
        imageUpload(storage_path('tests/listings/barbeque-12.jpg'), $oListing, ImageType::MODEL, [], [], false);

        $oListing = $this->factoryListingLocationWithAddress($oListing, [
            'address' => '23 Moody Place, New York, NY 10310, United States of America',
        ]);
        ...
        $guestEmail = 'guest-3@admin.com';
        $this->guestWithReservation($guestEmail, $oListing, 7, 5);

        $oListing->refresh();
        $oHost->refresh();

        $guestEmail = 'guest-31@admin.com';
        $this->guestWithReservation($guestEmail, $oListing, 9, 4);

//        $guestEmail = 'guest-32@admin.com';
//        $this->guestWithReservation($guestEmail, $oListing, 5);
//
//        $guestEmail = 'guest-33@admin.com';
//        $this->guestWithReservation($guestEmail, $oListing, 6);
//
//        $guestEmail = 'guest-34@admin.com';
//        $this->guestWithReservation($guestEmail, $oListing, 2);
//
//        $guestEmail = 'guest-35@admin.com';
//        $this->guestWithReservation($guestEmail, $oListing, 3);
    }
     ...
     * @param int $subDays
     * @param int $ratingToListing
     * @throws \Stripe\Exception\ApiErrorException
     */
    private function guestWithReservation(string $guestEmail, Listing $oListing, int $subDays, int $ratingToListing)
    {
        ...
        $data = [
            'start_at' => now()->subDays($subDays)->startOfDay()->addHours(6)->format(UserReservationServiceModel::DATE_FORMAT),
            'finish_at' => now()->subDays($subDays)->startOfDay()->addHours(8)->endOfHour()->format(UserReservationServiceModel::DATE_FORMAT),
            'price' => $price,
        ...
        $oListing->reviews()->create([
            ...
            'published_at' => now()->subDays($subDays)->endOfDay(),
        ]);
